...
export class Rail {
	...
	public railLength = 10;
	private railHeight = 0.8; // Increased height for better visibility
	private railWidth = 0.15; // Increased width for better visibility
	...
	private createRailMesh(): void {
		...
		const midpoint = new THREE.Vector3()
			.addVectors(this.startPosition, this.endPosition)
			.multiplyScalar(0.5);
	...
	private createRailSupports(): void {
		...
		for (const pos of supportPositions) {
			...
			const supportPosition = new THREE.Vector3().lerpVectors(
				this.startPosition,
				this.endPosition,
				pos,
			);
	...
	public calculateAttractionForce(position: THREE.Vector3): THREE.Vector3 {
		...
		const horizontalDistance = Math.sqrt(
			(position.x - closestPoint.x) ** 2 + (position.z - closestPoint.z) ** 2,
		);
		...
		if (
			horizontalDistance > maxHorizontalRange ||
