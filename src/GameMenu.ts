...
export class GameMenu {
	...
	private createMenuElement(): void {
		...
		this.menuElement.className =
			"fixed inset-0 flex justify-center items-center bg-gradient-to-br from-gray-900 to-black z-100 font-sans";
		...
		this.serverNameInput.className =
			"w-full p-3.5 rounded-lg bg-gray-700 border border-gray-600 text-white text-base focus:outline-none focus:ring-2 focus:ring-red-400 focus:border-transparent transition";
		...
		this.serverListContainer.className =
			"mb-5 rounded-lg overflow-hidden bg-gray-900/70 border border-gray-700 shadow-lg";
		...
		serverListTitle.className =
			"py-3.5 px-5 font-bold text-gray-200 bg-gray-800/90 border-b border-gray-700 flex justify-between items-center";
		...
		paginationContainer.className =
			"flex justify-between items-center py-3 px-5 bg-gray-800/60 border-t border-gray-700";
		...
		refreshButton.className =
			"w-full py-3 px-5 bg-indigo-600/70 hover:bg-indigo-700/70 text-white rounded-b-lg transition font-medium flex justify-center items-center gap-3";
		...
		this.peerCodeDisplay.className =
		...
		this.startButton.className =
			"bg-blue-600 hover:bg-blue-700 text-white py-4 px-6 rounded-lg font-bold text-lg transition-colors shadow-lg";
		this.startButton.textContent = "Start Game";
		buttonGroup.appendChild(this.startButton);

		this.resumeButton = document.createElement("button");
		this.resumeButton.type = "button";
		this.resumeButton.className =
			"bg-green-600 hover:bg-green-700 text-white py-4 px-6 rounded-lg font-bold text-lg transition-colors shadow-lg hidden";
	...
	private async refreshServerList(): Promise<void> {
		...
		try {
			...
			const renderServerEntry = (server: ServerEntry, isSelected: boolean) => {
				...
				serverEntry.className =
					"border-b border-gray-700 last:border-0 transition cursor-pointer hover:bg-gray-800/50";
	...
	private async startGame(): Promise<void> {
		...
		if (
			isOnlineMode &&
			this.hostRoleRadio.checked &&
			this.serverNameInput.value.trim()
		) {
			try {
				// Import dynamically to avoid circular dependencies
				const { addServer } = await import("./supabase");
				serverId =
					(await addServer(this.serverNameInput.value.trim())) || undefined;

				if (!serverId) {
					console.warn(
						"Failed to register server, continuing without server registration",
					);
				}
			} catch (error) {
		...
		const options: GameStartOptions = {
			...
			isOnlineMode,
			peerCode,
			serverName: this.hostRoleRadio.checked
				? this.serverNameInput.value.trim()
				: undefined,
			serverId: this.clientRoleRadio.checked
	...
	public showHostWaitingScreen(): void {
		...
		this.peerCodeDisplay = document.createElement("div");
		this.peerCodeDisplay.className =
			"bg-gray-900/80 text-yellow-300 p-6 rounded-lg font-mono text-2xl mb-8 border border-gray-700 mx-auto max-w-md";
		...
		cancelButton.className =
			"bg-gray-600 hover:bg-gray-500 text-white py-3 px-6 rounded-lg font-medium text-lg transition-colors shadow-lg";
